...
const AddNewModal = ({ open, handleModal, getData, currentPage, setCurrentPage, rowsPerPage, groupType }) => {
  ...
  const addNew = (data) => {
    const dataSubmit = {
      ...
      permissionGroupType: data?.permissionGroupType.value,
      description: data?.description ?? ""
    }
    createGroupPermission(dataSubmit)
      .then((res) => {
        MySwal.fire({
  ...
  return (
    ...
    >
      <ModalHeader
        ...
        tag="div"
      >
        <h4 className="modal-title">Thêm mới nhóm quyền</h4>
      </ModalHeader>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Input
                    ...
                    invalid={errors.permissionGroupName && true}
